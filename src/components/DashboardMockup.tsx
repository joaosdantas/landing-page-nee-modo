...
const processes = [
  ...
  { client: 'Vila Nova Office', operation: 'Empreendimento', status: 'Documentação', owner: 'Paula R.' },
]

const pipelineStages = [
  { name: 'Documentação', count: 8, width: 'w-[80%]', tone: 'bg-sky-400' },
  { name: 'Análise', count: 5, width: 'w-[62%]', tone: 'bg-amber-400' },
  { name: 'Validação', count: 4, width: 'w-[48%]', tone: 'bg-violet-400' },
  { name: 'Repasse', count: 3, width: 'w-[34%]', tone: 'bg-emerald-400' },
  { name: 'Registro', count: 2, width: 'w-[20%]', tone: 'bg-slate-400' },
]
...
export default function DashboardMockup() {
  return (
    <div
      role="img"
      aria-label="Interface ilustrativa do painel gerencial do Nee MODO com métricas, processos e pipeline"
      className="overflow-hidden rounded-2xl border border-white/10 bg-ink-800 shadow-2xl shadow-black/60 ring-1 ring-white/5"
    >
      <div className="flex items-center gap-2 border-b border-white/10 bg-ink-700/60 px-4 py-3">
        ...
        <span className="ml-auto flex items-center gap-3">
          <span className="hidden items-center gap-2 rounded-md bg-white/[0.06] px-3 py-1.5 text-[11px] text-slate-500 md:flex">
            <Search className="h-3.5 w-3.5" aria-hidden="true" />
            Buscar cliente, operação...
          </span>
      ...
      <div className="grid lg:grid-cols-[14rem_1fr]">
        <aside className="hidden flex-col border-r border-white/10 p-4 lg:flex">
          ...
          <nav className="mt-2 flex flex-col gap-1" aria-hidden="true">
            ...
            <span className="mt-auto flex items-center gap-2.5 rounded-lg px-3 py-2 pt-2 text-xs font-medium text-slate-500">
              <Settings className="h-4 w-4" />
              Configurações
            </span>
          </nav>
          <div className="mt-4 flex items-center gap-2.5 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2.5">
        ...
        <div className="space-y-5 p-5 sm:p-6" aria-hidden="true">
          ...
          <div className="grid gap-5 xl:grid-cols-[1.5fr_1fr]">
            <div className="overflow-hidden rounded-xl border border-white/10 bg-white/[0.02]">
              <div className="border-b border-white/10 px-4 py-3">
                <p className="text-xs font-semibold text-white">Processos recentes</p>
              </div>
              <div className="hidden grid-cols-[1.3fr_0.9fr_0.9fr_0.8fr] gap-3 border-b border-white/5 px-4 py-2.5 text-[10px] font-medium uppercase tracking-wider text-slate-500 sm:grid">
                <span>Cliente</span>
                <span>Operação</span>
                <span>Etapa</span>
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 sm:p-5">
              <p className="text-xs font-semibold text-white">Pipeline de operações</p>
                ...
                {pipelineStages.map((stage) => (
                  <div key={stage.name}>
                    ...
                    <div className="mt-1.5 h-1.5 rounded-full bg-white/10">
                      <div className={cn('h-full rounded-full', stage.tone, stage.width)} />
                    </div>
                  </div>
                ))}
              </div>
